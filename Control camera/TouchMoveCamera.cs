...
public class TouchMoveCamera : MonoBehaviour
{
    ...
    public float CameraLeftBound = 40;
    public float CameraRightBound = 40;
    public float CameraUpBound = 50;
    public float CameraDownBound = 20;
    public static TouchMoveCamera touchMoveCameraClass;
    ...
    private void Awake()
    {
        ...
        if (touchMoveCameraClass != null)
        {
            return;
        }
        else
        {
            touchMoveCameraClass = this;
        }
    ...
    private void Update()
    {
        ...
        Vector3 camPositionBeforeAdjustment1 = Camera.transform.position;
        //Scroll (Pan function)
        if (Input.touchCount >= 1)
        {
           ...
           Camera.transform.Translate(Delta1, Space.World);
            ...
            if (Camera.transform.position.x > (cameraStartPosition.x + CameraRightBound))
            {
                Camera.transform.position = camPositionBeforeAdjustment1;
                Debug.Log("Right Bound");
                return;
            }
            if (Camera.transform.position.x < (cameraStartPosition.x - CameraLeftBound))
            {
                Camera.transform.position = camPositionBeforeAdjustment1;
            ...
            if (Camera.transform.position.z > (cameraStartPosition.z + CameraUpBound))
            {
                Camera.transform.position = camPositionBeforeAdjustment1;
                Debug.Log("Up Bound");
                return;
            }
            if (Camera.transform.position.z < (cameraStartPosition.z - CameraDownBound))
            {
                Camera.transform.position = camPositionBeforeAdjustment1;
                Debug.Log("Down Bound");
